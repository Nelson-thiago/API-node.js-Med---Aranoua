...
export const alterarPessoa = async (req, res) => {
  try {
    const { nome, telefone, email, cidade, estado } = req.body;
    const id = req.params.id;
    ...
    const pessoaAtualizada = await Pessoa.update({
      ...
      telefone,
      email,
      cidade_id: cidadeEncontrada.id,
    },{
      where: { id: req.params.id }
    });
    res.status(200).json("Pessoa Alterada",pessoaAtualizada);
...
export const deletarPessoa = async (req, res) => {
  try {
    ...
    res.status(204).json("pessoa deletada");
